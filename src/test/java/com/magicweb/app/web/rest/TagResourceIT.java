...
import com.magicweb.app.repository.TagRepository;
import com.magicweb.app.service.criteria.TagCriteria;
import com.magicweb.app.service.dto.TagDTO;
import com.magicweb.app.service.mapper.TagMapper;
...
class TagResourceIT {
    ...
    @Test
    @Transactional
    void getAllTagsByArticlesIsEqualToSomething() throws Exception {
        WebArticle articles;
        if (TestUtil.findAll(em, WebArticle.class).isEmpty()) {
            tagRepository.saveAndFlush(tag);
            articles = WebArticleResourceIT.createEntity(em);
        } else {
            articles = TestUtil.findAll(em, WebArticle.class).get(0);
        }
        em.persist(articles);
        em.flush();
        tag.addArticles(articles);
        tagRepository.saveAndFlush(tag);
        Long articlesId = articles.getId();

        // Get all the tagList where articles equals to articlesId
        defaultTagShouldBeFound("articlesId.equals=" + articlesId);

        // Get all the tagList where articles equals to (articlesId + 1)
        defaultTagShouldNotBeFound("articlesId.equals=" + (articlesId + 1));
    }
